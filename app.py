...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql+psycopg2://apr_stds:419462@localhost:5432/t2w2sat"
...
def get_product(product_id):
    ...
    if product:
        ...
        return {"Error": f"Product with product_id {product_id} not found"}
...
def delete_product(product_id):
    ...
    if not is_admin:
        return {"Error": "Only an admin can delete products"}, 403
    ...
    if product:
        ...
        db.session.commit()
        return {"Success": f"Product with id {product_id} deleted"}
    else:
